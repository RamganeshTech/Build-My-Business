...
const Careers = () => {
    ...
    const validateStep = () => {
        ...
        if (step === 1) {
            ...
            if (!form.salary) e.salary = "Expected salary is required";
        }
        if (step === 2) {
        ...
        if (step === 4) {
            if (!form.about) e.about = "Tell us about yourself";
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setTimeout(() => {
                setIsSuccess(false);
                setStep(0);
                setForm({ fullName: "", phone: "", city: "", workType: "", workMode: "", salary: "", academic: "", skills: "", languages: "", projects: "", about: "", address: "" });
            }, 5000);
        } catch (err) {
                            ...
                            >
                                {step === 0 && (
                                    <>
                                        <Field label="Full Name" error={errors.fullName}>
                                            <input type="text" value={form.fullName} onChange={(e) => handleChange("fullName", e.target.value)} placeholder="John Doe" className={inputClass(errors.fullName)} />
                                        </Field>
                                        <Field label="Phone Number" error={errors.phone}>
                                            <input type="tel" maxLength={10} value={form.phone} onChange={(e) => handleChange("phone", e.target.value.replace(/\D/g, ""))} placeholder="93639 64498" className={inputClass(errors.phone)} />
                                        </Field>
                                ...
                                {step === 1 && (
                                    <>
                                        ...
                                        <Field label="Expected Salary (Monthly)" error={errors.salary}>
                                            <input type="text" value={form.salary} onChange={(e) => handleChange("salary", e.target.value.replace(/\D/g, ""))} placeholder="INR" className={inputClass(errors.salary)} />
                                        </Field>
                                    </>
                                )}
                                ...
                                {step === 2 && (
                                    <>
                                        <Field label="Key Skills & Strengths" error={errors.skills}>
                                            <textarea value={form.skills} onChange={(e) => handleChange("skills", e.target.value)} placeholder="List your professional skills..." className={inputClass(errors.skills) + " h-32"} />
                                        </Field>
                                ...
                                {step === 3 && (
                                    <Field label="Academic Records (10th, 12th, UG, PG)" error={errors.academic}>
                                        <textarea
                                            value={form.academic}
                                            onChange={(e) => handleChange("academic", e.target.value)}
                                            placeholder="Provide your score/CGPA for:&#10;10th:&#10;12th:&#10;UG:&#10;PG:"
                                            className={inputClass(errors.academic) + " h-64"}
                                        />
                                    </Field>
                                )}
                                ...
                                {step === 4 && (
                                    <>
                                        <Field label="Tell us about yourself" error={errors.about}>
                                            <textarea value={form.about} onChange={(e) => handleChange("about", e.target.value)} placeholder="Brief summary of your background..." className={inputClass(errors.about) + " h-32"} />
                                        </Field>
                                        <Field label="Permanent Address" error={errors.address}>
                                            <textarea value={form.address} onChange={(e) => handleChange("address", e.target.value)} placeholder="Complete residential address" className={inputClass(errors.address) + " h-24"} />
                                        </Field>
                                    </>
                                )}
                            </motion.div>
